...
const Pomodoro = () => {
  ...
  const resetTimer = (newMode = mode) => {
    setActive(false);
    setMode(newMode);
    if (newMode === 'work') setSecondsLeft(WORK_TIME);
    if (newMode === 'short') setSecondsLeft(SHORT_BREAK);
    if (newMode === 'long') setSecondsLeft(LONG_BREAK);
  };
  ...
  return (
    <div className="min-h-screen pb-24 bg-gradient-to-b from-purple-50 to-purple-200 flex flex-col items-center px-4">
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg font-semibold ${
            mode === 'work'
              ...
              : 'bg-white border border-purple-400 text-purple-700'
          }`}
        >
          Work
        </button>
      ...
      />
      <button type="submit" className='bg-purple-700 mb-5 flex align-self-end cursor-pointer text-white px-4 py-2 rounded-lg shadow hover:bg-purple-800 transition' onClick={handleSaveTask}>Save</button>
